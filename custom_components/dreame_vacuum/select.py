...
This module provides select entities for Dreame vacuum robots,
allowing users to choose cleaning modes, suction levels, and water volumes.
"""
...
SELECTS: tuple[DreameVacuumSelectEntityDescription, ...] = (
    ...
    DreameVacuumSelectEntityDescription(
        ...
        exists_fn=lambda description, device: bool(
            not device.capability.carpet_recognition and default_exists_fn(description, device)
        ),
    ),
    DreameVacuumSelectEntityDescription(
        ...
        exists_fn=lambda description, device: (
            ...
            or (device.capability.mop_pad_lifting_plus and default_exists_fn(description, device))
        ),
    ),
    DreameVacuumSelectEntityDescription(
    ...
    DreameVacuumSelectEntityDescription(
        ...
        value_int_fn=lambda value, device: int(value[0]),
        exists_fn=lambda description, device: (
            not device.capability.mop_clean_frequency and default_exists_fn(description, device)
        ),
    ...
    DreameVacuumSelectEntityDescription(
        ...
        exists_fn=lambda description, device: bool(
            device.capability.custom_mopping_route and default_exists_fn(description, device)
        ),
    ),
    DreameVacuumSelectEntityDescription(
    ...
    DreameVacuumSelectEntityDescription(
        ...
        exists_fn=lambda description, device: bool(
            device.capability.self_clean_frequency and default_exists_fn(description, device)
        ),
    ),
    DreameVacuumSelectEntityDescription(
        ...
        exists_fn=lambda description, device: bool(
            device.capability.auto_recleaning and default_exists_fn(description, device)
        ),
    ),
    DreameVacuumSelectEntityDescription(
        ...
        value_int_fn=lambda value, device: DreameVacuumSecondCleaning[value.upper()].value,
        exists_fn=lambda description, device: bool(
            device.capability.auto_rewashing and default_exists_fn(description, device)
        ),
    ),
    DreameVacuumSelectEntityDescription(
        ...
        exists_fn=lambda description, device: bool(
            device.capability.cleaning_route and default_exists_fn(description, device)
        ),
    ),
    DreameVacuumSelectEntityDescription(
        ...
        exists_fn=lambda description, device: bool(
            device.capability.cleangenius and default_exists_fn(description, device)
        ),
    ),
    DreameVacuumSelectEntityDescription(
    ...
    DreameVacuumSelectEntityDescription(
        ...
        exists_fn=lambda description, device: bool(
            device.capability.water_temperature and default_exists_fn(description, device)
        ),
    ),
    DreameVacuumSelectEntityDescription(
        key="auto_empty_mode",
    ...
    DreameVacuumSelectEntityDescription(
        ...
        exists_fn=lambda description, device: (
            device.capability.self_wash_base and device.capability.mop_clean_frequency
        ),
    ),
    DreameVacuumSelectEntityDescription(
        key="washing_mode",
...
SEGMENT_SELECTS: tuple[DreameVacuumSelectEntityDescription, ...] = (
    ...
    DreameVacuumSelectEntityDescription(
        ...
        exists_fn=lambda description, device: (
            device.capability.customized_cleaning and not device.capability.self_wash_base
        ),
        segment_list_fn=lambda device: device.status.current_segments,
    ),
    DreameVacuumSelectEntityDescription(
        ...
        exists_fn=lambda description, device: (
            device.capability.customized_cleaning and device.capability.self_wash_base
        ),
        segment_list_fn=lambda device: device.status.current_segments,
    ),
    DreameVacuumSelectEntityDescription(
        ...
        segment_available_fn=lambda device, segment: bool(
            ...
            and not device.status.cruising
            and not device.status.cleangenius_cleaning
            and not device.status.started  # Cannot change per-room settings while cleaning
        ),
        ...
        exists_fn=lambda description, device: (
            device.capability.customized_cleaning and device.capability.custom_cleaning_mode
        ),
        segment_list_fn=lambda device: device.status.current_segments,
    ...
    DreameVacuumSelectEntityDescription(
        ...
        value_int_fn=lambda value, device: DreameVacuumCustomMoppingRoute[value.upper()].value,
        exists_fn=lambda description, device: (
            device.capability.segment_mopping_settings and not device.capability.cleaning_route
        ),
        segment_list_fn=lambda device: device.status.current_segments,
    ),
    DreameVacuumSelectEntityDescription(
        ...
        value_fn=lambda device, segment: FLOOR_MATERIAL_DIRECTION_CODE_TO_NAME.get(
            (
                ...
                if segment.floor_material_rotated_direction is not None
                    ...
                    if device.status.current_map is None
                    or device.status.current_map.rotation == 0
        ...
        value_int_fn=lambda value, self: next(
            (
                ...
                for name, type in self.segment.name_list(
                    self.device.status.segments, self.coordinator.hass.config.language
                ).items()
                if name == value
            ),
            None,
...
class DreameVacuumOptionNavigationMixin:
    """Index-based option navigation shared by the select entities.

    Operates on the standard SelectEntity attributes (``_attr_options`` /
    ``_attr_current_option``) and ``async_select_option``. Mixed in before
    SelectEntity so the navigation behaves identically for the plain and the
    per-segment select entities.
    """
...
class DreameVacuumSelectEntity(DreameVacuumOptionNavigationMixin, DreameVacuumEntity, SelectEntity):  # type: ignore[misc]
    ...
    entity_description: DreameVacuumSelectEntityDescription
    ...
    def __init__(
        ...
        self._computed_set_fn: Callable[[Any, int, int], None] | None = description.set_fn
        self._computed_options_fn: Callable[[Any, Any], list[str]] | None = description.options
        ...
        if self._computed_options_fn is not None:
            self._attr_options = self._computed_options_fn(coordinator.device, None)
    ...
    def _handle_coordinator_update(self) -> None:
        if self.device is not None:
            if self._computed_options_fn is not None:
                self._attr_options = self._computed_options_fn(self.device, None)
            self._attr_current_option = self.native_value
        ...
        if option not in (self._attr_options or []):
            raise HomeAssistantError(
                ...
                translation_placeholders={
                    ...
                    "option": option,
                    "options": str(self._attr_options),
                },
            )
...
class DreameVacuumSegmentSelectEntity(DreameVacuumOptionNavigationMixin, DreameVacuumEntity, SelectEntity):  # type: ignore[misc]
    ...
    def __init__(
        ...
        if description.name == "" and description.options is not None:
            self._segment_options_fn = lambda device, segment: list(
                segment.name_list(device.status.segments, self.coordinator.hass.config.language)
            )
        ...
        if self.segment:
            if self._segment_options_fn is not None:
                self._attr_options = self._segment_options_fn(coordinator.device, self.segment)
            self._attr_current_option = self.native_value
    ...
    def _set_id(self) -> None:
        ...
        if self.entity_description.name == "":
            # Special "name" select for room renaming
        ...
        if self.segments != device_segments:
            ...
            if self.segments and self.segment_id in self.segments:
                if self.segment != self.segments[self.segment_id]:
                    ...
                    self._set_id()
                if self._segment_options_fn is not None:
                    self._attr_options = self._segment_options_fn(self.device, self.segment)
            elif self.segment:
                self._attr_options = []
                self.segment = None
                self._set_id()
