...
This module provides button entities for Dreame vacuum robots,
such as reset consumables, locate device, and start cleaning actions.
"""
...
from .entity import DreameVacuumButtonEntityDescription, DreameVacuumEntity, default_exists_fn
...
BUTTONS: tuple[DreameVacuumButtonEntityDescription, ...] = (
    ...
    DreameVacuumButtonEntityDescription(
        ...
        exists_fn=lambda description, device: bool(
            default_exists_fn(description, device) and device.capability.auto_empty_base
        ),
    ),
    DreameVacuumButtonEntityDescription(
...
def async_update_buttons(
    ...
    if coordinator.device.capability.shortcuts:
        ...
        for shortcut_id in new_ids - current_ids:
            ...
            new_entities.extend(current_shortcut[shortcut_id])
    ...
    if coordinator.device.capability.backup_map:
        ...
        for map_index in new_indexes - current_ids:
            ...
            new_entities.extend(current_map[map_index])
...
class DreameVacuumButtonEntity(DreameVacuumEntity, ButtonEntity):
    ...
    entity_description: DreameVacuumButtonEntityDescription
...
class DreameVacuumShortcutButtonEntity(DreameVacuumEntity, ButtonEntity):
    ...
    __slots__ = ("id", "shortcut", "shortcut_id", "shortcuts")
    ...
    def __init__(
        ...
        # self.id must be set BEFORE super().__init__(): the inherited constructor
        # calls _set_id(), whose fallback branch (no matching shortcut) reads
        # str(self.id). Setting it afterwards raised AttributeError in that case.
        self.id = shortcut_id
        if self.id >= 32:
            self.id = self.id - 31
        ...
        self.entity_id = async_generate_entity_id(
            ENTITY_ID_FORMAT,
            f"{self.device.name} shortcut_{self.id}",
    ...
    def _set_id(self) -> None:
        ...
        if self.shortcut:
            shortcut_name = self.shortcut.name
            if shortcut_name.lower().startswith("shortcut"):
                shortcut_name = shortcut_name[len("shortcut") :].lstrip()
        else:
            shortcut_name = str(self.id)
        self._attr_translation_key = "shortcut_named"
        self._attr_translation_placeholders = {"name": shortcut_name}
    ...
    def extra_state_attributes(self) -> dict[str, str] | None:
        ...
        return self.shortcut.as_dict() if self.shortcut is not None else None
...
class DreameVacuumMapButtonEntity(DreameVacuumEntity, ButtonEntity):
    ...
    def __init__(
        ...
        self.entity_id = async_generate_entity_id(
            ENTITY_ID_FORMAT,
            f"{self.device.name} backup_map_{self.map_index}",
            hass=self.coordinator.hass,
        )
    ...
    def _set_id(self) -> None:
        ...
        if self._map_name is None:
            map_display = str(self.map_index)
        else:
            name_str = self._map_name.replace("_", " ").replace("-", " ")
            map_display = name_str[0].upper() + name_str[1:] if name_str else name_str
        self._attr_translation_key = "backup_map"
        self._attr_translation_placeholders = {"map_name": map_display}
    ...
    async def async_press(self, **kwargs: Any) -> None:
        ...
        if map_data is None:
            raise HomeAssistantError(
                translation_domain=DOMAIN,
                translation_key="map_data_unavailable",
            )
        await self._try_command(
            "Unable to call %s",
