...
This module provides sensor entities for Dreame vacuum robots, including
battery level, cleaning time, cleaned area, and various status sensors.
"""
...
from .dreame.vacuum_types import ATTR_ROOM_ICON, ATTR_ROOM_ID
from .entity import DreameVacuumEntity, DreameVacuumSensorEntityDescription, default_exists_fn
...
SENSORS: tuple[DreameVacuumSensorEntityDescription, ...] = (
    ...
    DreameVacuumSensorEntityDescription(
        ...
        exists_fn=lambda description, device: (
            device.capability.camera_streaming or default_exists_fn(description, device)
        ),
    ),
    DreameVacuumSensorEntityDescription(
    ...
    DreameVacuumSensorEntityDescription(
        ...
        entity_category=EntityCategory.DIAGNOSTIC,
        exists_fn=lambda description, device: bool(
            default_exists_fn(description, device) and device.capability.deodorizer
        ),
    ),
    DreameVacuumSensorEntityDescription(
    ...
    DreameVacuumSensorEntityDescription(
        ...
        value_fn=lambda value, device: (
            datetime.fromtimestamp(value, tz=datetime.now().astimezone().tzinfo) if value else None
        ),
    ),
    DreameVacuumSensorEntityDescription(
...
class DreameVacuumSensorEntity(DreameVacuumEntity, SensorEntity):
    ...
    entity_description: DreameVacuumSensorEntityDescription
    ...
    def __init__(
        ...
        # Compute value_fn before calling super().__init__ which stores in _computed_value_fn
        # We need to set _computed_value_fn after super().__init__ if needed
        super().__init__(coordinator, description)

        # Override _computed_value_fn to target the "_name" display variant when present.
        self._computed_value_fn = self._resolve_value_fn(coordinator, description, "_name")
