...
### Changed
- **Faster HA startup** (measured on a real device: integration setup
  4.25 s → 2.46 s). The first device update no longer re-requests every
  property (`force_all`) while the persisted-inventory background thread is
  already loading the deferred set, and the map manager (numpy/PIL + ~20 MB
  of base64 resource data) is now constructed lazily from a worker thread on
  first connect instead of on the HA event loop during coordinator init.
- Dependency floors refreshed: **numpy 2.5.1**, pillow 12.3.0,
  pycryptodome 3.23.0, `actions/stale` v11, `actions/setup-python` v7,
